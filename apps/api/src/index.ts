...
if (!process.env.DATABASE_URL) {
  app.log.error("DATABASE_URL no estÃ¡ definida. Revisa tu .env en el root del repo.");
}
...
// MenÃºs activos reales desde BD
...
async function createMenuHandler(body: CreateMenuBody, reply: any) {
  ...
  if (!body?.title || body.title.trim().length < 3) {
    return reply.code(400).send({ error: "title es obligatorio (mÃ­n 3 caracteres)" });
  }
  if (!Number.isFinite(body.priceCents) || body.priceCents <= 0) {
    return reply.code(400).send({ error: "priceCents debe ser nÃºmero > 0" });
  }
  if (!Number.isFinite(body.quantity) || body.quantity <= 0) {
    return reply.code(400).send({ error: "quantity debe ser nÃºmero > 0" });
  }
...
app.post("/api/orders", async (req: any, reply: any) => {
  ...
  if (!menu) {
    return reply.code(404).send({
      ...
      message: "La oferta no existe, no estÃ¡ activa o no tiene stock",
    });
  }
  ...
  try {
    ...
    if (String(e?.message ?? "").includes("OUT_OF_STOCK")) {
      return reply.code(409).send({
        error: "OUT_OF_STOCK",
        message: "Sin stock (alguien reservÃ³ justo antes)",
      });
    }
