  ...
  return (
    <section className="space-y-6">
      <header className="space-y-2">
        ...
        <h1 className="text-3xl font-bold text-slate-900">Mis liquidaciones</h1>
        <p className="text-sm text-zinc-500">Resumen de pagos por periodo</p>
      </header>

      <div className="flex flex-wrap gap-2">
        <Link href="/r" className="inline-flex items-center justify-center rounded-xl border border-zinc-200 bg-white px-4 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50">
          Panel
        </Link>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Card className="space-y-1">
          <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Total acumulado</p>
      ...
      {loading ? (
        ...
        <Card className="p-4"><p className="text-sm text-zinc-500">Aún no hay liquidaciones.</p></Card>
      ) : (
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <button onClick={() => viewDetail(s)}
                    className="mt-1 text-xs font-semibold text-zinc-500 hover:text-zinc-900">
                    Ver detalle
                  </button>
                </div>
              </div>
      ...
      {detail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setDetail(null)}>
          <div className="w-full max-w-2xl max-h-[80vh] overflow-auto rounded-2xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-zinc-500">{formatDate(detail.periodStart)} → {formatDate(detail.periodEnd)}</p>
            <div className="mt-3 grid grid-cols-3 gap-3 text-center">
              ...
              <div className="rounded-xl bg-emerald-50 p-3">
                <p className="text-xs text-zinc-500">Tu ingreso neto</p>
            ...
            <div className="mt-4 flex justify-end">
              <button onClick={() => setDetail(null)}
                className="rounded-xl border border-zinc-200 bg-white px-4 py-2 text-sm font-semibold text-zinc-700">Cerrar</button>
            </div>
          </div>
        </div>
      )}
